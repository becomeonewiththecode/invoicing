...
export function AdminDashboardPage() {
  ...
  const metrics = [
    ...
    { label: 'Total Invoices', value: stats?.totalInvoices ?? '-', color: 'text-primary' },
    {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        {metrics.map((m) => (
          <div key={m.label} className="bg-surface rounded-lg shadow p-6">
      ...
      <div className="bg-surface rounded-lg shadow p-6 mb-8">
        <h2 className="text-lg font-semibold text-text mb-4">User Growth (Last 30 Days)</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {health?.services.map((svc) => {
          ...
          return (
            <div key={svc.name} className="bg-surface rounded-lg shadow p-6">
              <div className="flex items-center gap-3 mb-2">
                <span className={`inline-block w-3 h-3 rounded-full ${icon.color}`} />
                <h3 className="font-semibold text-text capitalize">{svc.name}</h3>
              </div>
              <p className="text-sm text-text-muted">{icon.label} &middot; {svc.responseTimeMs}ms</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-surface rounded-lg shadow p-6">
          ...
          <p className="text-2xl font-bold text-primary">{health?.avgResponseTime ?? '-'}ms</p>
        </div>
        <div className="bg-surface rounded-lg shadow p-6">
          <p className="text-sm text-text-muted">Requests (Last Hour)</p>
          <p className="text-2xl font-bold text-primary">{health?.requestsLastHour ?? '-'}</p>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-lg shadow p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-text">System Logs</h2>
          <select
            value={logLevel}
            onChange={(e) => { setLogLevel(e.target.value); setLogPage(1); }}
            className="rounded border border-input-border px-3 py-1 text-sm"
          >
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-border text-sm">
            <thead className="bg-surface-alt">
              <tr>
                <th className="px-4 py-2 text-left text-xs font-medium text-text-muted uppercase">Time</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-text-muted uppercase">Level</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-text-muted uppercase">Method</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-text-muted uppercase">Path</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-text-muted uppercase">Status</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-text-muted uppercase">Time (ms)</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-text-muted uppercase">IP</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {logs?.data.map((log) => (
                <tr key={log.id} className="hover:bg-surface-alt">
                  <td className="px-4 py-2 text-text-muted whitespace-nowrap">{new Date(log.created_at).toLocaleTimeString()}</td>
                  ...
                  <td className="px-4 py-2 text-text-secondary max-w-xs truncate">{log.path || '-'}</td>
                  <td className="px-4 py-2 text-text-secondary">{log.status_code || '-'}</td>
                  <td className="px-4 py-2 text-text-secondary">{log.response_time_ms ?? '-'}</td>
                  <td className="px-4 py-2 text-text-muted">{log.ip || '-'}</td>
                </tr>
              ))}
              {logs && logs.data.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-4 text-center text-text-muted">No logs found</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {logs && logs.pagination.total > logs.pagination.limit && (
          <div className="flex items-center justify-between border-t border-border px-4 py-3 mt-2">
            <p className="text-sm text-text-muted">Page {logPage} of {Math.ceil(logs.pagination.total / logs.pagination.limit)}</p>
