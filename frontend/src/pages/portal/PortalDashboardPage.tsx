    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
          ...
          <p className="text-2xl font-bold tabular-nums mt-1 text-blue-700">{stats.openInvoices}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
          <p className="text-sm font-medium text-gray-500">Outstanding balance</p>
          <p className="text-2xl font-bold tabular-nums mt-1 text-amber-700">
            {money(stats.outstandingTotal)}
          </p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
          <p className="text-sm font-medium text-gray-500">Visible invoices</p>
      ...
      <section>
        ...
        {notifQuery.isPending && <p className="text-gray-400">Loading activity…</p>}
        {notifQuery.isError && <p className="text-red-600">Could not load activity.</p>}
        {notifQuery.data && notifQuery.data.data.length === 0 && (
          <p className="text-gray-500 py-4">No recent invoice or project updates.</p>
        )}
        {notifQuery.data && notifQuery.data.data.length > 0 && (
            ...
            {notifQuery.data.data.slice(0, 15).map((n) => (
              <li key={n.id} className="px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                <div>
                  ...
                  <p className="text-sm text-gray-600">{n.detail}</p>
                </div>
